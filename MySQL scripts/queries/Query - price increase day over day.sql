...
WHERE p.percentage > 0
AND (t.price_today - y.price_yesterday) > .03 -- optional filter for minimum price change (eliminates .01 to .02 = 100% price change)
AND qoh.quantity_on_hand > 0 -- optional filter for quantity on hand. 
-- ORDER BY percent DESC -- optional order by highest percent. Must comment out if not using
ORDER BY price_difference DESC -- optional order by largest price difference. Must Comment out if not using
;
